...
P=pathlib.Path.cwd();out={'tests':[],'errors':[],'network':[]};Q=P/'qa';(Q/'functional').mkdir(exist_ok=True)
...
with sync_playwright() as pw:
 ...
 page.set_viewport_size({'width':1440,'height':900});seek(118);out['final_state']=state();out['gpu']=page.evaluate('window.__B52.gpu');check('No JavaScript or shader errors',not out['errors'] and not state()['shaderErrors']);check('No external runtime fetches',not out['network']);browser.close()
out['pass_count']=sum(t['pass'] for t in out['tests']);out['total']=len(out['tests']);(Q/'pass2_functional.json').write_text(json.dumps(out,ensure_ascii=False,indent=2),encoding='utf-8');print('TOTAL',out['pass_count'],'/',out['total'])
